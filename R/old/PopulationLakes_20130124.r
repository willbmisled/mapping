...
#start loop
B<-748  #row number to start processing lakes; this will be 1 to start.
#N<-nrow(Lakes)    #last row to process; 
N<-750
S<-100                 #save the work every "S" lakes
Counter<-0             #counter; should be zero to start
LakePop<-data.frame(matrix(NA,nrow=(N-B+1)*4,ncol=6))  #build data.frame to store the results
...
#Start Loop 
for(i in c(B:N)){
LakePop[(4*(i-B+1)-3):((i-B+1)*4),]<-try(PopDensity(Lakes[i,],'N'),silent=T)     #replce with tryCatch
Counter<-Counter+1
...
#load("L:/Public/Milstead_Lakes/RData/PopulationLakes_20130124.rda")
