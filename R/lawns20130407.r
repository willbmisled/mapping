...
#Get lawn raster
#Lawns database from Cristina Milesi reprojected to:
  #ESRI USA_Contiguous_Albers_Equal_Area_Conic_USGS_version
  #see LawnsRasterAlbers20130407.r
  load('L:/Public/Milstead_Lakes/LawnCoverCristinaMilesi20130212/LawnsRasterAlbers20130407.rda')  #filename = LawnsAlbers
  plot(LawnsAlbers)

#LawnsAlbers is the %lawn cover for all lower 48 states.  Reduce to bbox(MRB1Lakes) + 20km
    B<-bbox(MRB1Lakes)
    Extent<-c(B[1,1]-20000,B[1,2]+20000,B[2,1]-20000,B[2,2]+20000)
  #Get Impervious data for the Largest Lake Buffer + 3 30x30 pixels
    Lawns<-crop(LawnsAlbers,Extent)
    plot(Lawns)
    plot(MRB1Lakes,add=T)

#save the data
File<-paste('L:/Public/Milstead_Lakes/RData/Data_',v,'da',sep='')  #file to store data:  v is the version (see top of file)
save(Lawns,MRB1Lakes,file=File)  #save the last records.
#load("L:/Public/Milstead_Lakes/RData/Data_lawns20130407.rda")   

writeRaster(Lawns, filename='C:/Bryan/EPA/Data/LawnCoverCristinaMilesi20130212/Lawns20130407.tif', format="GTiff", overwrite=TRUE)
  


################################
WB_ID<-sample(MRB1Lakes$WB_ID,1) #random lake
Lake<-MRB1Lakes[match(WB_ID,MRB1Lakes$WB_ID),]  #select lake
 ...
 par(mfrow=c(1,2))
   Mask<-mask(Lawn, Buf2500)  #extract impervious cover pixels in the buffer (all others changed to NA)

   plot(Mask)
   str(plot)
table(getValues(Mas),useNA='ifany')
